...
export default function DeliveryForm({
  ...
  const [unloaderName, setUnloaderName] = useState("");
  const [unloaderTitle, setUnloaderTitle] = useState("Unloading Foreman");
  ...
  const [foremanRole, setForemanRole] = useState("Unloading Foreman");
  ...
  useEffect(() => {
    if (lastDelivery) {
      ...
      if (u) {
        setUnloaderId(u.unloaderId || "");
        setUnloaderName(u.unloaderName || "");
        setUnloaderTitle(u.unloaderTitle || "Unloading Foreman");
        setEquipmentType(u.equipmentType || "");
        setCapacity(u.capacity ? String(u.capacity) : "");
        setEquipmentPlateNo(u.equipmentPlateNo || "");
        setOperatorName(u.operatorName || "");
        setOperatorId(u.operatorId || "");
      }
    }
  }, [lastDelivery]);
  ...
  // Save/Submit the entire receiving transaction
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trailerNo.trim()) {
      setErrorList("Trailer/Vehicle Number is required.");
      return;
    }
    if (!mdrNo.trim()) {
      setErrorList("MDR Slip Number is required.");
      return;
    }
    ...
    try {
      ...
      for (const item of items) {
        ...
        const deliveryPayload = {
          ...
          mdrNo: mdrNo.trim().toUpperCase(),
          trailerNo: trailerNo.trim().toUpperCase(),
          ...
          flatNo: "",
          unloadingDetails: {
            unloaderId: unloaderId.trim(),
            unloaderName: unloaderName.trim(),
            unloaderTitle: foremanRole,
      ...
      const metaPromises = [
        saveSuggestion("mdrNo", mdrNo),
        saveSuggestion("trailerNo", trailerNo),
      ...
      setRemarks("");
      // Clear specific elements details but keep sticky trailer/mdr/employee details
      setMdrNo("");
      setTrailerNo("");

      setSuccessList(`Successfully recorded ARA Ledger: All ${items.length} products logged and synchronized under MDR ${mdrNo.toUpperCase()}!`);
      onSuccess();
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (err) {
      console.error("Error logging bulk products:", err);
      setErrorList("Error saving elements to Firestore. Check connection or project settings.");
      handleFirestoreError(err, OperationType.WRITE, "deliveries");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              ...
              <option value="Storekeeper" className="bg-slate-900">Storekeeper</option>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
          <CustomCombobox
            ...
            value={mdrNo}
            onChange={setMdrNo}
            suggestions={suggestions.mdrNo || []}
            placeholder="e.g. MDR-09381"
          />
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400 font-bold">
            Total items: <span className="text-blue-400">{items.length}</span>
          </span>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="damage" className="bg-slate-900 text-amber-400">Damage (Needs Repair)</option>
                      <option value="reject" className="bg-slate-900 text-rose-400">Rejected (To Supplier)</option>
                    </select>
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input
                      ...
                      placeholder={isRejected ? "Provide reason (e.g. hair cracks, chip-off)..." : "Not applicable (Good condition)"}
